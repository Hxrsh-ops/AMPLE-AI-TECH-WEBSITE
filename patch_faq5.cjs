...
const regex = /function initFaqAccordion\(\) \{[\s\S]*?\}\s*function initStatsZoom\(\)/;
...
const newFaq = `function initFaqAccordion() {
    ...
    const activeColors = {
        bg: 'var(--token-53ac5080-809d-430f-8c22-cd6ed2fb990c, #0d0d0d)',
    ...
    function resetAll() {
        faqContainers.forEach(container => {
            // Clean up older classes that might force active state
            if (container.classList.contains('framer-v-63nu9f')) {
                container.classList.remove('framer-v-63nu9f');
                container.classList.add('framer-v-m8cjo8');
            }
            ...
            // bgTop/bgBottom hiding to avoid conflicts
    ...
    faqContainers.forEach(container => {
        ...
        headerPart.addEventListener('click', (e) => {
            ...
            // Set active styles
            container.style.backgroundColor = activeColors.bg;
    ...
    if (firstQuestion && faqAnswers[firstQuestion]) {
        // Just trigger click on the first one to initialize state
        const firstHeader = faqContainers[0].querySelector('.framer-bvehib') || faqContainers[0];
...
console.log('Fixed FAQ logic per user screenshot');
